            ...
            try {
                ...
                for (Element link : links) {
                    ...
                    if (temp.startsWith("https://")) {
                        // Validate link
                        if (v.isValid(link.attr("href")) && v.isValid2(link.attr("href"))) {
                            flag = true;
                            innerValid++;

                            System.out.println("\033[32mVALID \033[0m");

                        } else if (!v.isValid(link.attr("href")) || !v.isValid2(link.attr("href"))) {
                        ...
                        String tempDomain = v.getDomain(url);
                        tempUrl = v.combine(tempDomain, temp); // Fix the url by adding " https:// "

                        // Validate the link
                        if (v.isValid(tempUrl) && v.isValid2(tempUrl)) {
                            innerValid++;
                            System.out.println("\033[32mVALID (else)\033[0m");
                            flag = true;

                        } else if (!v.isValid(tempUrl) || !v.isValid2(tempUrl)) {
                            innerInvalid++;
